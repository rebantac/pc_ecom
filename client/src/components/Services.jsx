...
const Services = () => {
  ...
  return (
    <div
      id="services"
      className="mt-12 max-w-[1490px] mx-auto px-10 flex flex-row items-center xs:flex-col xs:justify-between w-full"
    >
      {services.map((service, i) => (
        <FadeIn key={i} delay={0.2} direction="down">
          <div className="flex flex-row gap-4 m-4 items-center xs:items-start">
            <img
              src={service.icon}
              className="max-h-[84px] max-w-[84px]"
              alt=""
            />
            <div className="flex flex-col gap-1.5 text-center xs:text-left">
